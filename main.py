...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-c', '--csv', 
        default='/mnt/dfs/qtim/clu/files/rop-2021-07-01.csv',
...
#         'epochs_per_round': 4,
    ...
    if args.fed:
        ...
        fed_dir = run_dir / 'federated_checkpoints'
        fed_dir.mkdir(exist_ok=True, parents=True)
    ...
    if args.debug:
        ...
        valid_df = valid_df.sample(frac=0.20)
        test_df = test_df.sample(frac=0.20)
    ...
    if args.fed:
        ...
        train_ds = MONAI_DS(train_df, example='segment', augment=True)
        valid_ds = MONAI_DS(valid_df, example='segment', augment=False)
    ...
    class_prop = [1 / v for v in train_df.label.value_counts().tolist()]
    class_prop = np.array(class_prop)
    if args.fed:
        fed_weights = {
            site: class_prop[site_df[site_df.split == 'train'].label.values]
            for site, site_df in  fed_df.items()
        }
        fed_batch_sampler = {
        ...
        weights = class_prop[train_df.label.values]
    ...
    if args.fed:
        ...
        for r in range(2 if args.debug else param['rounds']):
            ...
            torch.save({
                ...
                'round': r,
            }, str(run_dir / f'global_model_{r}.pth'))
        ...
        for site, site_dl in test_dl.items():
            ...
            with open(run_dir / f'{site}-test.json', 'w') as f:
        ...
        plot_curve(train_metric, valid_metric, 
                   ...
                   ylim=(0, 1),
                   save_path = run_dir / 'metric_curve.png',
                  )
